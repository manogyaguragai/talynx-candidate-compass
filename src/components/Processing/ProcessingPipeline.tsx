...
export const ProcessingPipeline: React.FC = () => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-slate-200 dark:border-gray-700 p-6 mb-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg font-semibold font-inter text-slate-900 dark:text-white">Processing Pipeline</h2>
        <div className="flex items-center space-x-2">
          <Clock className="w-4 h-4 text-slate-400 dark:text-gray-400" />
          <span className="text-sm font-fira text-slate-600 dark:text-gray-300">
      ...
      <div className="relative">
        {/* Progress bar */}
        <div className="absolute top-5 left-5 right-5 h-0.5 bg-slate-200 dark:bg-gray-600">
          <div 
              ...
              <div key={stage.id} className="flex flex-col items-center relative">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${
                  status === 'complete' 
                    ...
                    : 'bg-white dark:bg-gray-700 border-slate-300 dark:border-gray-600 text-slate-400 dark:text-gray-400'
                }`}>
                  <StageIcon className="w-5 h-5" />
                </div>
                
                <div className="mt-3 text-center">
                  <p className="text-sm font-medium font-inter text-slate-900 dark:text-white">{stage.label}</p>
                  <p className="text-xs text-slate-500 dark:text-gray-400 font-ibm">{stage.description}</p>
      ...
      {processing.isProcessing && processing.currentStage !== 'complete' && (
        <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          <div className="flex items-center space-x-2">
            <div className="w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>
            <span className="text-sm font-ibm text-primary dark:text-blue-300">
